...
sidebar_label: powerpipe check
---



# powerpipe check
Execute one or more Powerpipe benchmarks and controls.

You may specify one or more benchmarks or controls to run, or run `powerpipe check all` to run all controls in the workspace.

## Usage
Run benchmarks/controls:
```bash
powerpipe check [item,item,...] [flags]
```

List available benchmarks:
```bash
powerpipe check list
```
...
<table>
  ...
  <tr> 
    ...
    <td>  Help for <inlineCode>powerpipe check.</inlineCode></td> 
  </tr>
  ...
  <tr>
    ...
    <td> Set the maximum number of database connections to open. When running powerpipe check, Powerpipe will attempt to run up to this many controls in parallel.  See the <a href="reference/env-vars/powerpipe_max_parallel">STEAMPIPE_MAX_PARALLEL</a> environment variable documentation for details. (default <inlineCode>10</inlineCode>) </td> 
  </tr>
  ...
  <tr> 
    ...
    <td>  Filter the list of controls to run, using a sql <inlineCode>where</inlineCode> clause against the <inlineCode>powerpipe_control</inlineCode> reflection table. 
    </td>
  </tr>
...
| `snapshot` | Powerpipe snapshot json (alias for `sps`)
| `sps` | Powerpipe snapshot json.
...
```bash
powerpipe check --snapshot benchmark.cis_v130
```

Run a benchmark and share a [snapshot](/docs/snapshots/batch-snapshots):
```bash
powerpipe check --share benchmark.cis_v130
```
...
```bash
powerpipe check benchmark.cis_v130 --tag cis_level=1 --dry-run
```

Run controls with the a benchmark=pci tag that are either high or critical severity:
```bash
powerpipe check all --where "severity in ('critical', 'high') and tags ->> 'pci' = 'true'"
```
...
```bash
powerpipe check benchmark.foundational_security --where "tags ->> 'foundational_security_item_id' !=  all(ARRAY['cloudformation_1','s3_11'])"
```
...
```bash
powerpipe check all --export=csv --export=output.json
```

Export to markdown and json with default file names, asff as `output.asff.json`, nunit3 as `output.nunit3.xml`

```bash
powerpipe check all --export=md --export=json --export=output.asff.json --export=output.nunit3.xml
```

Send json output to stdout and pipe  to `jq `
```bash
powerpipe check all --output=json | jq
```
