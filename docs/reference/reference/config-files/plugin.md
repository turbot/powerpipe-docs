...
In a `connection` you may continue to use the current syntax for the `plugin` argument. Powerpipe will resolve the `connection` to the `plugin` as long as they resolve to the same plugin version:
